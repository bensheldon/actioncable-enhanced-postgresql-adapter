...
- Rename the reliable broadcasting channel param from `since` to `enhanced-since` (also accepting `enhanced_since`, what turbo-rails produces from a `data-enhanced-since` attribute). The view helper is renamed to match: `action_cable_since_param` is now `action_cable_enhanced_since_param`. `enhanced-since` is a plain ISO 8601 UTC timestamp, not encrypted - it isn't secret, it only ever selects a window of messages the subscriber is already authorized to receive, and `messages_since` clamps it to `message_retention` before running the query, so a client can never request more history than the server retains
